...
class ItemRepository implements ItemRepositoryInterface {
  ...
  Future<List<Item>?> _getPopularItemList(String type,
    ...
    switch (source) {
      case DataSourceEnum.client:
        String url = '${AppConstants.popularItemUri}?type=$type';
        print('Siddhesh Request URL: $url');
        Response response = await apiClient.getData(url);
        print('Siddhesh Request Headers: ${apiClient.getHeader()}');
        print('Siddhesh Response Status Code: ${response.statusCode}');
        print('Siddhesh Response Body: ${response.body}');
      ...
      case DataSourceEnum.local:
        print('Siddhesh Fetching from local with cacheId: $cacheId');
        String? cacheResponseData = await LocalClient.organize(
            DataSourceEnum.local, cacheId, null, null);
        print('Siddhesh Cached Response: $cacheResponseData');
        if (cacheResponseData != null) {
          popularItemList = [];
          popularItemList
    ...
    if (kDebugMode) {
      print("Siddhesh Popular Item $popularItemList");
    }
